...
$stmt->execute();
if ($stmt->rowCount() > 0) {
    header('Location: ../index.php?type=error&message=' . $username . ' is already exist');
    exit;
}
    ...
    $sql = "INSERT INTO `students` (user_id, `firstname`, `lastname`, `birthdate`, `email`, `department`, `course`, `civil`, `graduated`, `children`, `phone`, `present_address`, `work_address`) 
        ...
        header('Location: ../../index.php?type=success&message=Successfully Registered - Please wait for the approval of the administrator');